"""
Process raw training data to create filtered training data.

This script processes raw training data from a Parquet file and filters it 
based on various criteria. It handles the removal of NAs, filtering of the last 
5% of data, and merging with labeled events. The final output is a processed 
dataset with event annotations (0 for no event, 1 for onset, 2 for wakeup). 
Additionally, the script computes and saves the maximum step for each series_id 
and removes participants with excessive missing data.

Usage:
    python process_train_data.py
"""

__authors__ = "Eliott Tempez, Nadezhda Zhukova"
